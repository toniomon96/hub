...
- **v0.5 #3: SSE streaming for `/api/ask`.** New `runStream()` async generator in `@hub/agent-runtime` yields typed events (`meta | token | final | error`) — Ollama path streams OpenAI-compatible deltas, Anthropic path forwards assistant message chunks from the SDK. Caller may pass an `AbortSignal`; aborting mid-stream finalizes the run as `partial` with `errorMessage='aborted'`. New `POST /api/ask/stream` endpoint emits the events as Server-Sent Events frames (`text/event-stream`, no buffering headers for nginx-friendly deployments). New `hub ask --stream` CLI flag pipes tokens to stdout incrementally; Ctrl-C aborts the upstream generator. The non-streaming `POST /api/ask` endpoint is unchanged for clients that still want a single JSON response. Event contract lives in `@hub/shared/contracts/ask.ts` as `AskStreamMeta | AskStreamToken | AskStreamFinal | AskStreamError`.
