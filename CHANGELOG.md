...
- **v0.5 #8: 80% spend pre-warning via ntfy.** New `publishNtfy()` helper in `@hub/shared` (no-op when `NTFY_TOPIC` is unset, never throws). `packages/db/src/spend.ts` exposes `getSpendState()` / `todayDateKey()` so callers can derive thresholds from a single snapshot. A new `apps/server/src/spend-warn.ts` task fires exactly one ntfy per calendar day when today's Anthropic spend crosses 80% of `HUB_DAILY_USD_CAP`; dedup'd via a marker file at `<data-dir>/.spend-warning-<YYYY-MM-DD>.flag`. Wired into `cron.ts` on a `*/15 * * * *` schedule (only when `HUB_BRIEF_ENABLED=1`). The 100% hard stop in router rule 3 is unchanged.
