...
`packages/prompts/src/edit.ts` provides three mutation functions — `addTarget`, `wirePrompt`, `removeEntry` — exposed via CLI (`hub registry add|wire|remove|list`), HTTP (`POST /api/registry/add`, `/api/registry/wire`, `/api/registry/remove`, `GET /api/registry/targets`), and MCP (`hub.registry.add`, `hub.registry.wire`, `hub.registry.remove`, `hub.registry.list`).
